...
寫死於 `include/.../gd_solver.hpp` 成員預設值。
...
| `K_inner_` | 200 | hpp:158 | 內層 GD 最大迭代 |
...
| `run_solver_global(traj, targets, Xa_opt, Xb_opt, solver_log)` | 用 5 點中的 3 個內部點組成決策變數 X0，建立並呼叫 `GdSolver::run_alm()` |
...
### 10-2. `gd_solver`（第 1 層：內層 ALM + GD 求解器）

| 函式 | 說明 |
|------|------|
| `GdSolver(X, robotA_base, robotB_base, danger_threshold, path_weight, smooth_w...)` | 建構子：接收 (P+2)×12 控制點矩陣與機器人/問題參數，計算維度 `M_/num_D_/num_X_/num_C_` |
| `run_alm()` | 對外主要入口：跑 ALM 外層（罰參數/乘子排程）+ GD 內層（方向 `d=-G`，1D Newton 線搜索）直到收斂/發散/達上限，回傳 `SolverLog` |
...
| `compute_D_cache(X, D_base, D_plus)` | 中心差分快取：算 D 的基準值與正向擾動值，供梯度共用（GD 不需 Hessian，故只需正向擾動）|
| `compute_G_smooth(X)` | 平滑項對 X 的梯度（`num_X_` 維）|
| `compute_G_c(X, D_base, D_plus, mu_loc, c_loc)` | 完整梯度 `G = G_smooth + 罰項梯度`，即最陡下降方向 `d=-G` 的來源 |
...
| `DualArmAlmGdPlannerManager::initialize(model, ns)` | 插件初始化：從 node 讀取 yaml 參數存入成員 |
