...
使用 C++17 + Eigen3 實作。內層採用 **ALM（增廣拉格朗日）+ CG-FR（Fletcher-Reeves 共軛梯度）+ 1D Newton 線搜索**。
...
### 內層：ALM 共軛梯度求解器 (`CgSolver::run_alm`)
...
│  │   ┌── 內層 CG-FR 迭代 (最多 alm_k_inner=200 步) ────────────┐ │    │
...
- **`config/`**：**執行期參數層**——存放 move_group 啟動時讀入的 yaml 參數，需複製為 `hiwin_dual_arm/config/dual_arm_alm_cg_1_planning.yaml`。
- **`include/dual_arm_alm_cg_planner_1/`**：**介面宣告層**。
- **`src/`**：**演算法實作層**。
...
| `dual_arm_alm_cg_planner_1.xml` | pluginlib 插件描述（讓 MoveIt2 動態載入） |
| `README.md` | 本文件 |
| `PARAMETERS.md` | 參數對照表（yaml 可調 / 程式內建） |
| `config/dual_arm_alm_cg_planner_1.yaml` | 規劃器參數（需複製為 `hiwin_dual_arm/config/dual_arm_alm_cg_1_planning.yaml`） |
| `cg_solver.hpp/.cpp` | 第 1 層：FK、包覆球、危險因子、ALM + CG-FR 內層求解 |
...
1. 把本包 `config/dual_arm_alm_cg_planner_1.yaml` 複製為
   `hiwin_dual_arm/config/dual_arm_alm_cg_1_planning.yaml`（檔名依規劃器而不同）。
...
對外唯一入口：`export_unified(prefix, level)`。輸出到 `prefix/<timestamp>_CG/` 目錄（每次匯出自成一個時間戳資料夾，不互相覆蓋）。
...
2. **機器人底座**（`avoidance_system.cpp` 建構函數）：目前 A 臂 `Ty(700)Rz(180)`、B 臂 `Ty(-700)`，兩臂相距 1400mm 面對面 — 依實際擺位調整。
3. **包覆球參數**（`cg_solver.cpp` 的 `BUBBLES_*` / `PEDESTAL_*` 常數）：RA610/RA605 的球座標與半徑，依實際機型確認。
