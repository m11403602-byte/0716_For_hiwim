...
| 參數 | 預設 | 說明 |
|------|------|------|
| `export_csv_prefix` | `""`（關）| 非空 → 每次規劃後匯出 CSV 子目錄 `<prefix>/<unix秒>_CG/` |
...
### 2-2. 危險因子（`cg_solver.cpp` `calc_df`）
...
| `run_solver_global(traj, targets, Xa_opt, Xb_opt, solver_log)` | 用 5 點中的 3 個內部點組成決策變數 X0，建立並呼叫 `CgSolver::run_newton()` |
...
### 4-2. `cg_solver`（第 1 層：純 Lagrangian 共軛梯度求解器）

| 函式 | 說明 |
|------|------|
| `CgSolver(X, robotA_base, robotB_base, danger_threshold, path_weight, smooth_w...)` | 建構子：接收 (P+2)×12 控制點矩陣與機器人/問題參數，計算維度 `M_/num_D_/num_X_/num_C_/total_dim_`，並呼叫 `rebuild_initial_V_()` 建 `V_0=[X_0;λ_0;S_0]` |
| `run_newton()` | 對外主要入口：跑 CG-FR 迭代（方向 `d=-G` 或 `d=-G+β·d_prev`，1D Newton 線搜索）直到 `phys_ok && stable_ok` 或達 `lag_max_iter`，回傳 `SolverLog`（函式名沿用內層主迴圈命名，非全維度 Newton）|
...
| `compute_D_cache(V, D_base, D_plus, D_minus)` | 中心差分快取：算 D 的基準值與正負向擾動值，供梯度使用（CG 不需 Hessian）|
| `compute_G_smooth(V)` | 平滑項對 X 的梯度 `∇_X f` |
| `compute_G(V, D_base, D_plus)` | 完整 KKT 一階殘差 `G=[G_X;G_λ;G_S]`（`total_dim_` 維），即 CG 方向要驅近 0 的量 |
...
| `DualArmLagCgPlannerManager::initialize(model, ns)` | 插件初始化：從 node 讀取 yaml 參數存入成員 |
