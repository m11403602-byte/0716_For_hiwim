...
- **回傳碼**：`0` = 成功避障，`1` = 仍碰撞。
- CSV 輸出在 `myout/<unix秒>_CG/` 資料夾下（`export_unified` 統一匯出，level 1=6 檔、
...
`hiwin_dual_arm/config/dual_arm_lag_cg_1_planning.yaml` 已經寫好：
...
1. `hiwin_dual_arm/package.xml` 加 `<exec_depend>dual_arm_lag_cg_planner_1</exec_depend>`。
2. `move_group.launch.py`（或想用的 launch 檔）用
   `MoveItConfigsBuilder(...).planning_pipelines(pipelines=["ompl", "dual_arm_lag_cg_1", ...])`
   把 `dual_arm_lag_cg_1` 加進清單。
3. 重新編譯：
   ```bash
...
`dual_arm_lag_cg_1`，設定起訖姿態後按 Plan / Execute。

### 3.4 調參數（不用重編，改 yaml 後重啟 `move_group`）

編輯 `config/dual_arm_lag_cg_planner_1.yaml`（或
`hiwin_dual_arm/config/dual_arm_lag_cg_1_planning.yaml`，視實際載入路徑而定，
兩者欄位相同）。也可以在 `move_group` 已啟動時動態改（免重啟）：

```bash
ros2 param set /move_group dual_arm_lag_cg_1.danger_threshold 0.35
```
...
| RViz 下拉選單看不到 `dual_arm_lag_cg_1` | 六個 pipeline 還沒接進 launch，見第 3.2 節 / `hiwin_dual_arm/README.md` 第 2 節 |
