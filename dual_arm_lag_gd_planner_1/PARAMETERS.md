# 參數對照表 — dual_arm_lag_gd_planner_1
...
### 2-1. 內層收斂與線搜索（`gd_solver.cpp`）

| 參數 | 值 | 說明 |
|------|-----|------|
| `delta` | 0.01 | 有限差分擾動 h（FK 與 cost FD 共用）|
| 線搜索 `LS_DELTA` | **0.01** | 一維函數 φ(α) 的有限差分步長|
| 線搜索 `MAX_INNER` | **2000** | 一維 Newton 線搜索允許的最大內層迭代次數|
| `H_MIN_ABS` | 1e-9 | 二階退化保護 |
| `ANEW_TOL` | 1e-3 | 步長收斂門檻 |
| `FALLBACK_ALPHA` | 0.0001 | 線搜索失敗退路 |
| 發散門檻 | ‖G‖ > 1e9 或 NaN/Inf | |

### 2-2. 危險因子（`gd_solver.cpp` `calc_df`）
...
### 2-4. 機器人幾何（`gd_solver.cpp`，與 ALM/Newton 譜系位元一致）
...
| `run_solver_global(traj, targets, Xa_opt, Xb_opt, solver_log)` | 用 5 點中的 3 個內部點組成決策變數 X0，建立並呼叫 `GdSolver::run_newton()` |
...
### 4-2. `gd_solver`（第 1 層：純 Lagrangian 最陡下降求解器）

| 函式 | 說明 |
|------|------|
| `GdSolver(X, robotA_base, robotB_base, danger_threshold, path_weight, smooth_w...)` | 建構子：接收 (P+2)×12 控制點矩陣與機器人/問題參數，計算維度 `M_/num_D_/num_X_/num_C_/total_dim_`，並呼叫 `rebuild_initial_V_()` 建 `V_0=[X_0;λ_0;S_0]` |
| `run_newton()` | 對外主要入口：跑最陡下降迭代（方向 `d=-G`，1D Newton 線搜索）直到 `phys_ok && stable_ok` 或達 `lag_max_iter`，回傳 `SolverLog`（函式名沿用內層主迴圈命名，非全維度 Newton）|
...
| `compute_D_cache(V, D_base, D_plus, D_minus)` | 中心差分快取：算 D 的基準值與正負向擾動值，供梯度使用（GD 不需 Hessian）|
| `compute_G_smooth(V)` | 平滑項對 X 的梯度 `∇_X f` |
| `compute_G(V, D_base, D_plus)` | 完整 KKT 一階殘差 `G=[G_X;G_λ;G_S]`（`total_dim_` 維），即最陡下降方向 `d=-G` 的來源 |
...
| `line_search_newton_1d(V, d, ...)` | 沿方向 `d` 做 1D Newton 線搜索求步長 `alpha`（`LS_DELTA=0.01`）|
...
| `DualArmLagGdPlannerManager::initialize(model, ns)` | 插件初始化：從 node 讀取 yaml 參數存入成員 |
