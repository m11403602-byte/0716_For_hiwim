# dual_arm_lag_newton_planner_1 — 使用手冊

本文件是「怎麼操作」的手冊：編譯、跑起來、切模式、調參數、排除常見錯誤。
演算法原理與數學推導請見 [README.md](README.md)；逐項參數的預設值與程式碼位置請見
[PARAMETERS.md](PARAMETERS.md)。

一句話定位：內層用 **純 Lagrangian 模型 + 全維度 Newton 法**求解雙臂避障問題（決策變數
`V=[X;λ;S]`，與 ALM 系列是不同的數學模型，不可混用 yaml/常數）。
...
- 收斂判定是 `phys_ok && stable_ok && stat_ok`（本包唯一有啟用 `stat_ok` 的純 Lagrangian
  版本）。若 `has_collision` 一直是 `true`：先加大 `max_refinement_iter` 或
  `lag_max_iter`，再調 `danger_threshold` / `path_weight`；若 Newton 內層本身不穩定
  （不定 KKT 矩陣），可考慮改 `newton_solver.cpp` 內 `H.fullPivLu().solve(G)`
  （見 README.md「已知限制」）。
...
`hiwin_dual_arm/config/dual_arm_lag_newton_1_planning.yaml` 已經寫好：
...
1. `hiwin_dual_arm/package.xml` 加 `<exec_depend>dual_arm_lag_newton_planner_1</exec_depend>`。
2. `move_group.launch.py`（或想用的 launch 檔）用
   `MoveItConfigsBuilder(...).planning_pipelines(pipelines=["ompl", "dual_arm_lag_newton_1", ...])`
   把 `dual_arm_lag_newton_1` 加進清單。
3. 重新編譯：
   ```bash
...
`dual_arm_lag_newton_1`，設定起訖姿態後按 Plan / Execute。

### 3.4 調參數（不用重編，改 yaml 後重啟 `move_group`）

編輯 `config/dual_arm_lag_newton_planner_1.yaml`（或
`hiwin_dual_arm/config/dual_arm_lag_newton_1_planning.yaml`，視實際載入路徑而定，
兩者欄位相同）。也可以在 `move_group` 已啟動時動態改（免重啟）：

```bash
ros2 param set /move_group dual_arm_lag_newton_1.danger_threshold 0.35
```
...
| `lag_s0` | 1.0 | S 決策變數初值 |
| `lag_tol_phys_margin` / `lag_tol_stable` / `lag_tol_stat` | 0.01/0.005/0.1 | 三條收斂容差（本包 `lag_tol_stat` 有作用） |
...
| RViz 下拉選單看不到 `dual_arm_lag_newton_1` | 六個 pipeline 還沒接進 launch，見第 3.2 節 / `hiwin_dual_arm/README.md` 第 2 節 |
| `has_collision` 一直是 `true` | 先加大 `max_refinement_iter`/`lag_max_iter`，再調 `danger_threshold`/`path_weight` |
| Newton 內層數值不穩（`‖G‖` 忽大忽小或發散） | KKT Hessian 為不定鞍點矩陣，`LDLT` 對此類矩陣可能不穩；可改 `newton_solver.cpp` 用 `H.fullPivLu().solve(G)`（見 README.md「已知限制」） |
