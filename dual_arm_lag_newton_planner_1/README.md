...
雙臂避障路徑規劃器 — ROS 2 Humble / MoveIt2 插件。**純 Lagrangian 模型 + 全維度 Newton 法**。
...
內層 = **全維度 Newton**：每步組 **1116×1116 的 9-block KKT Hessian** H，解 `d = −H⁻¹G`（Eigen LDLT），**α=1 純 Newton 步、無線搜索**。

⚠ **收斂判定 = `phys_ok && stable_ok && stat_ok`（Newton 啟用 `stat_ok`）**：Newton 解完整 KKT 系統，`‖G‖` 可被驅近 0，故 `stat_ok = (‖G‖ ≤ TOL_STAT=0.1)` 有意義。

⚠ **已知限制（不定 KKT）**：KKT Hessian 為不定鞍點矩陣，用 Eigen `LDLT` 求解對不定矩陣可能不穩（已知/預期，文件記載）。若需更穩健的分解，可改 `H.fullPivLu().solve(G)`（一行）。
...
| 組成 | 角色 |
|------|------|
| `newton_solver` | 第 1 層：內層 Newton 求解器 (含 compute_H) + FK + 包覆球 + 危險因子 |
...
│        │                                                            │
│        ▼                                                            │
│ max_D(t) > ＝ 碰撞門檻 (danger_threshold_ + collision_tolerance_ <= 0.5) 代表碰撞持續優化  ?  ──否──▶ 完成 (has_collision=false)│
│        │是                                                           │
│        ▼                                                            │
...
### 內層：純 Lagrangian 全維度 Newton 求解器 (`NewtonSolver::run_newton`)
...
│        │                                                           │
│        ▼                                                           │
│ ┌── Newton 迭代 (無外層 ALM 排程) ─────────────────────────────┐   │
│ │  計算殘差:                                                    │   │
│ │    G_X = ∇f + w_d Σ λ_i ∇D_i        (stationarity)           │   │
│ │    G_λ = w_d · (D - θ + S²)          (primal feasibility)     │   │
│ │    G_S = 2 w_d · S ⊙ λ              (complementarity)         │   │
│ │  組 9-block KKT Hessian H (1116×1116) = compute_H(V)          │   │
│ │  解 d = -H⁻¹G   (Eigen LDLT 分解)                              │   │
│ │  V ← V + alpha·d   (alpha = 1，純 Newton 步、無線搜索)          │   │
│ │        ▼                                                      │   │
│ │  收斂判定: phys_ok (max_D ≤ θ+margin) &&                       │   │
│ │            stable_ok (max_D 穩定) &&                           │   │
│ │            stat_ok (‖G‖ ≤ TOL_STAT=0.1)                       │   │
│ │      ──否──▶ 回到 Newton 迭代 (最多 lag_max_iter 步)            │   │
...
| `H` | 9-block KKT Hessian（不定鞍點矩陣） | 1116×1116 |
| `d` | Newton 方向 `-H⁻¹G` | 1116 維 |
| `alpha` | 步長，固定為 1（純 Newton 步、無線搜索） | scalar |
| `w_d` | Lagrangian 對約束項的權重（`lag_wd`） | scalar |
| `θ` | 危險因子閾值（同 `danger_threshold`） | scalar |
| `phys_ok` / `stable_ok` / `stat_ok` | 三條收斂判定：物理可行 / max_D 穩定 / KKT 平穩性 | bool |
...
- **`include/dual_arm_lag_newton_planner_1/`**：**介面宣告層**——用 `.hpp` 定義各層的類別、資料結構與函式簽名，決定模組之間如何互相呼叫，不含實作細節。
- **`src/`**：**演算法實作層**——對應 `include/` 中宣告的邏輯本體，是實際執行運算（FK、危險因子、純 Lagrangian 全維度 Newton 優化迭代、CSV 匯出、MoveIt 介面轉接）的地方。
- **`standalone/`**：**獨立驗證層**——提供不依賴 ROS/MoveIt runtime 的可執行進入點，用來單獨跑演算法、除錯、產生實驗資料，與正式的 MoveIt2 插件路徑互不影響。

四層資料夾的資料流方向大致是：`config/` 參數 → 經 `planner_manager`（第 3 層）讀入 → 呼叫 `avoidance_system`（第 2 層，外層碰撞修復）→ 呼叫 `newton_solver`（第 1 層，內層純 Lagrangian 全維度 Newton 優化）→ 結果透過 `data_io` 匯出 CSV，或轉換回 `RobotTrajectory` 交還 MoveIt。

```
dual_arm_lag_newton_planner_1/
├── CMakeLists.txt
├── package.xml
├── dual_arm_lag_newton_planner_1.xml
├── README.md
├── PARAMETERS.md
├── config/
│   └── dual_arm_lag_newton_planner_1.yaml
├── include/dual_arm_lag_newton_planner_1/
│   ├── newton_solver.hpp
...
| 檔案 | 功能 |
|------|------|
| `CMakeLists.txt` | 建置設定：把 `src/` 下四個 .cpp 編進單一共享庫 `libdual_arm_lag_newton_planner_1.so`，並設定 `-O3 -DNDEBUG`（刻意不含 `-march=native`，見下「編譯選項」）；另外編出 `run_standalone` 執行檔。 |
...
| `dual_arm_lag_newton_planner_1.xml` | pluginlib 的插件描述 XML，宣告 `DualArmLagNewtonPlannerManager` 類別與其 `base_class_type`（`planning_interface::PlannerManager`），讓 MoveIt2 能動態載入本插件。 |
| `README.md` | 本文件：數學模型、架構、編譯、使用方式、CSV 匯出格式、部署前調整項目。 |
| `PARAMETERS.md` | 參數對照表：列出所有外部可調（yaml/命令列）與內部寫死的參數、預設值、所在原始碼位置。 |
| `config/dual_arm_lag_newton_planner_1.yaml` | move_group 載入的規劃器參數：`path_weight`／`danger_threshold`、軌跡平滑權重、純 Lagrangian 內層參數（`lag_wd`／`lag_lam0`／`lag_s0`／`lag_tol_*`／`lag_max_iter`）、關節名前綴、時間參數化與 CSV 診斷輸出開關。 |
| `include/.../newton_solver.hpp` + `src/newton_solver.cpp` | **第 1 層**：純 Lagrangian 內層優化器。實作 FK（`transmatrix`）、雙臂包覆球模型（`BubbleDef` / `BUBBLES_*` / `PEDESTAL_*`）、危險因子 `calc_df`，以及決策變數 `V=[X;λ;S]` 的全維度 Newton 求解：組 1116×1116 的 9-block KKT Hessian `H`，用 Eigen `LDLT` 解 `d=-H⁻¹G`，`α=1` 純 Newton 步（無線搜索）。`compute_H` 負責組裝 KKT Hessian。回傳的 `SolverLog` 記錄每步 L、f、maxD、KKT 殘差（stationarity/primal/complementarity）等收斂歷程。 |
| `include/.../avoidance_system.hpp` + `src/avoidance_system.cpp` | **第 2 層**：外層碰撞修復迴圈（純數學，不依賴 MoveIt，可獨立使用）。定義 `Trajectory`（各臂關節角軌跡）、`CollisionIndices`（危險段 5 個控制點）等資料結構；`AvoidanceSystem` 類別串起「Clamped Spline 生成初始軌跡 → 碰撞偵測 → 找危險段 → 呼叫 Newton 內層優化 → Spline 局部重建 → 重新檢查」的完整流程，並提供 `export_unified` 統一 CSV 匯出函式。 |
...
2. **`avoidance_system.cpp`**（`AvoidanceSystem::run_optimization()`）— 外層碰撞修復迴圈；找到危險段後建構 `NewtonSolver` 並呼叫 `run_newton()`。
3. **`newton_solver.cpp`**（`NewtonSolver::run_newton()`）— 內層純 Lagrangian 全維度 Newton 優化迭代，各參數在這裡被實際用於計算（例如 `danger_threshold_` 用在約束項 `D_i(X) - danger_threshold_ + s_i²`）。
...
- 在 `avoidance_system.cpp` 看到它被存成成員、傳給 `NewtonSolver` 建構子，並在 `run_optimization()` 用來跟 `max_D` 比較判斷是否碰撞。
- 在 `newton_solver.cpp` 看到它被用在約束項 `D_i(X) - danger_threshold_ + s_i²`。
...
```bash
cd ~/ros2_ws
colcon build --packages-select dual_arm_lag_newton_planner_1
...
```bash
ros2 run dual_arm_lag_newton_planner_1 run_standalone myout
```
...
並載入 `config/dual_arm_lag_newton_planner_1.yaml`。
...
| `inner.csv` | 逐迭代完整軌跡（**22 欄**）：L/f/f_a/f_b/penalty/maxD/G_norm/d_norm/alpha + KKT 殘差 + λ/S 範圍 + **`cond_H`**（Newton 專屬，僅 verbose 計算）|
...
3. **包覆球**：`newton_solver.cpp` 的 `BUBBLES_*` / `PEDESTAL_*`。
