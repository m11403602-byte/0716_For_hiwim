...
#include "dual_arm_lag_newton_planner_1/data_io.hpp"
                                        ...
                                        SolverLog& solver_log)
{
  ...
  solver.set_lag_params(lag_wd_, lag_lam0_, lag_s0_,
                        lag_tol_phys_, lag_tol_stable_, lag_tol_stat_, lag_max_iter_);   // [NEW] yaml → 純 Lagrangian 注入
  solver.set_verbose(solver_verbose_);   // [NEW] yaml 開關透傳
  solver_log = solver.run_newton();
...
void AvoidanceSystem::export_unified(const std::string& prefix, int level) const
{
  ...
  // ---- 2. summary.csv: 每輪計分板 (純 Lagrangian; 每輪取末迭代值) ----
  {
    Eigen::MatrixXd S(n_outer, 14);
    for (int k = 0; k < n_outer; ++k) {
      const SolverLog& lg = iter_log_[k].solver_log;
      const int idx = std::max(1, lg.total_iter) - 1;   // 末迭代 (0-indexed)
      ...
      S(k,11) = last(lg.G_norm_history);   // 末步 ‖G‖ (鞍點不收斂, 供觀察)
      S(k,12) = last(lg.r_stat_history);   // 末步 stationarity 殘差
      S(k,13) = last(lg.kkt_history);      // 末步 combined KKT
    }
  ...
  // ---- 3. inner.csv: 逐迭代完整軌跡 (純 Lagrangian; 含成本拆解 + KKT 殘差 + λ/S) ----
  //   ⚠ 純 Lagrangian 為單一主迴圈 (無 ALM 的內/外兩層), 此處 step = 主迴圈迭代
  {
    int total = 0;
    ...
    for (int k = 0; k < n_outer; ++k) {
      ...
      for (int s = 0; s < lg.total_iter; ++s) {
        ...
        I(row,21) = g(lg.cond_H_history, s);   // [Newton] cond(H) (僅 verbose 非 0, 否則 NaN→以 0 記)
        ++row;
      }
    }
