...
`hiwin_dual_arm/config/dual_arm_alm_newton_1_planning.yaml` 已經寫好：
...
1. `hiwin_dual_arm/package.xml` 加 `<exec_depend>dual_arm_alm_newton_planner_1</exec_depend>`。
2. `move_group.launch.py`（或想用的 launch 檔）用
   `MoveItConfigsBuilder(...).planning_pipelines(pipelines=["ompl", "dual_arm_alm_newton_1", ...])`
   把 `dual_arm_alm_newton_1` 加進清單。
3. 重新編譯：
   ```bash
...
`dual_arm_alm_newton_1`，設定起訖姿態後按 Plan / Execute。

### 3.4 調參數（不用重編，改 yaml 後重啟 `move_group`）

編輯 `config/dual_arm_alm_newton_planner_1.yaml`（或
`hiwin_dual_arm/config/dual_arm_alm_newton_1_planning.yaml`，視實際載入路徑而定，
...
| `alm_mu0` / `alm_c0` / `alm_c_max` / `alm_beta_c` / `alm_gamma_v` | 10/5/100/2/0.5 | ALM 罰參數/乘子排程（Newton 版用溫和排程，勿照抄 CG/GD 的數值） |
...
| RViz 下拉選單看不到 `dual_arm_alm_newton_1` | 六個 pipeline 還沒接進 launch，見第 3.2 節 / `hiwin_dual_arm/README.md` 第 2 節 |
| `has_collision` 一直是 `true` | 依序調 `max_refinement_iter` → `danger_threshold`/`path_weight` → `alm_c_max`/`alm_beta_c`（見第 2.4 節） |
| ALM 路徑發散（數值爆掉） | 先降 `alm_beta_c`，再降 `alm_c_max`，最後才動 `alm_c0`（yaml 註解已寫建議順序） |
