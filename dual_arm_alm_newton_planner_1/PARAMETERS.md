...
本規劃器所有可調參數皆在 `config/dual_arm_alm_newton_planner_1.yaml`（本表 = yaml 註解 + 補充說明）。
修改流程：編輯 yaml → 複製為 `hiwin_dual_arm/config/dual_arm_alm_newton_1_planning.yaml` → 重啟 move_group 生效（**不用重編**）。
...
| `alm_c_max` | `2000.0` | 罰參數上限（三 solver 統一為 2000）。**調大**→深碰撞更壓得下去；**調小**→更安全但重疊深時可能卡住；⚠ 不宜過大：c 越大罰項 Hessian 越主導 → H 條件數惡化 → LDLT 解出的牛頓方向失準|
| `alm_gamma_v` | `0.5` | 放大觸發比 ‖V_k‖ > gamma_v·‖V_k-1‖（原始值 0.5，須在 0~1；程式內箝位至 (0,1)，非法值回落 0.5）。**調小**→更易觸發放大、c 長得快；**調大（接近 1）**→幾乎沒改善才放大、c 長得慢 |
| `alm_beta_c` | `2.0` | 罰參數放大係數 c ← beta_c·c（三 solver 統一為 2；須 > 1）。**調大**→罰爬升快、收斂快但易發散；**調小（接近 1）**→爬升慢、更穩但圈數變多；⚠ 不宜過大：c 放大過快會提早推高條件數，Newton 建議溫和排程 |
...
| 參數 | 預設 | 說明 |
|------|------|------|
| `export_csv_prefix` | `"./alm_data"` | 匯出根目錄：`<prefix>/<unix秒>_Newton/`（每次匯出自成時間戳資料夾，不互相覆蓋；export_level=0 時不匯出）|
