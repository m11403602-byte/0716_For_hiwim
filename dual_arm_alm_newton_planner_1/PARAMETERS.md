...
寫死於 `include/.../newton_solver.hpp` 成員預設值。
...
| `K_inner_` | 200 | hpp:158 | 內層 Newton 最大迭代 |
...
| `run_solver_global(traj, targets, Xa_opt, Xb_opt, solver_log)` | 用 5 點中的 3 個內部點組成決策變數 X0，建立並呼叫 `NewtonSolver::run_alm()` |
...
### 10-2. `newton_solver`（第 1 層：內層 ALM + Newton 求解器）

| 函式 | 說明 |
|------|------|
| `NewtonSolver(X, robotA_base, robotB_base, danger_threshold, path_weight, smooth_w...)` | 建構子：接收 (P+2)×12 控制點矩陣與機器人/問題參數，計算維度 `M_/num_D_/num_X_/num_C_` |
| `run_alm()` | 對外主要入口：跑 ALM 外層（罰參數/乘子排程）+ Newton 內層（組 Hessian、LDLT 求解）直到收斂/發散/達上限，回傳 `SolverLog` |
...
| `compute_D_cache(X, D_base, D_plus, D_minus)` | 中心差分快取：算 D 的基準值與正/負向擾動值，供梯度與 Hessian 共用，避免重複 FK |
| `compute_G_smooth(X)` | 平滑項對 X 的梯度（`num_X_` 維）|
| `compute_G_c(X, D_base, D_plus, mu_loc, c_loc)` | 完整梯度 `G = G_smooth + 罰項梯度`，即 Newton 內層要驅近 0 的量 |
| `cost_function_F(X)` / `cost_function_F_split(X, f, fa, fb)` | 平滑總成本，及拆解為 A/B 臂分量 |
| `cost_Xm(Xa, Xori, XH, XT)` | 單一控制點相對平滑參考的成本 |
| `cost_L_loc(X, mu_loc, c_loc)` | 局部 Lagrangian 值（成本 + ALM 罰項）|
| `compute_Dm_local(Xm_local)` | 由「單一控制點 12 維局部關節向量」重新算 D_m，供 Hessian 非對角雙擾動使用 |
| `alm_penalty_from_D(D_m, mu_m, c_loc)` / `alm_penalty_sum(Xm_local, mu_m, c_loc)` | 由危險因子計算 ALM 罰項純量（`psi = (1/2c)·Σ{[max(0,mu+c·g)]² − mu²}`）|
| `compute_H_smooth(X)` | 平滑項 Hessian（中心差分：對角用中心差分，非對角用前向-前向）|
| `compute_H_c(X, D_base, D_plus, D_minus, mu_loc, c_loc)` | 完整 Newton Hessian `H = H_smooth + H_collision`（block-diagonal 雙擾動組出碰撞項）|
...
| `DualArmAlmNewtonPlannerManager::initialize(model, ns)` | 插件初始化：從 node 讀取 yaml 參數存入成員 |
