...
使用 C++17 + Eigen3 實作。內層採用 **ALM（增廣拉格朗日）+ GD（最陡下降）+ 1D Newton 線搜索**。
...
### 內層：ALM 梯度下降求解器 (`GdSolver::run_alm`)
...
- **`config/`**：**執行期參數層**——存放 move_group 啟動時讀入的 yaml 參數，需複製為 `hiwin_dual_arm/config/dual_arm_alm_gd_1_planning.yaml`。
- **`include/dual_arm_alm_gd_planner_1/`**：**介面宣告層**。
- **`src/`**：**演算法實作層**。
...
| `dual_arm_alm_gd_planner_1.xml` | pluginlib 插件描述（讓 MoveIt2 動態載入） |
| `README.md` | 本文件 |
| `PARAMETERS.md` | 參數對照表（yaml 可調 / 程式內建） |
| `config/dual_arm_alm_gd_planner_1.yaml` | 規劃器參數（需複製為 `hiwin_dual_arm/config/dual_arm_alm_gd_1_planning.yaml`） |
| `gd_solver.hpp/.cpp` | 第 1 層：FK、包覆球、危險因子、ALM + GD 內層求解 |
...
1. 把本包 `config/dual_arm_alm_gd_planner_1.yaml` 複製為
   `hiwin_dual_arm/config/dual_arm_alm_gd_1_planning.yaml`（檔名依規劃器而不同）。
...
3. **包覆球參數**（`gd_solver.cpp` 的 `BUBBLES_*` / `PEDESTAL_*` 常數）：RA610/RA605 的球座標與半徑，依實際機型確認。
