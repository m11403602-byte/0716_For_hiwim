...
本規劃器所有可調參數皆在 `config/dual_arm_alm_gd_planner_1.yaml`（本表 = yaml 註解 + 補充說明）。
修改流程：編輯 yaml → 複製為 `hiwin_dual_arm/config/dual_arm_alm_gd_1_planning.yaml` → 重啟 move_group 生效（**不用重編**）。
...
| 參數 | 預設 | 說明 |
|------|------|------|
| `export_csv_prefix` | `"./alm_data"` | 匯出根目錄：`<prefix>/<unix秒>_GD/`（每次匯出自成時間戳資料夾，不互相覆蓋；export_level=0 時不匯出）|
